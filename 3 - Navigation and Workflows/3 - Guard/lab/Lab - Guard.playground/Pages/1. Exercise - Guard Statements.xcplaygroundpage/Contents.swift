...
func add(firstNumber: Int?, secondNumber: Int? ) -> Int? {
    guard let firstNum = firstNumber, let secondNum = secondNumber else {
        return nil
    }
    return firstNum + secondNum
}

add(firstNumber: 10, secondNumber: 2)
...
if let user = createUser() {
    print("First name: \(user.firstName)\r Last name: \(user.lastName)\r Age: \(user.age)")
}
